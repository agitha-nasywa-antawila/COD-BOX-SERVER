...
const orderNotComplateTemplate = () => {
    return `
        <a>
...
generalDataLoader({ url: "/api/v1/order/owner/order", func: renderTableRow });
...
searchForm.addEventListener("keyup", (e) => {
    if (String(searchForm.value).length > 3 || e.key == "Enter") {
        ...
        generalDataLoader({
            url: `/api/v1/order/owner/order?search=${searchValue}`,
            func: renderTableRow,
        });
    }
});
